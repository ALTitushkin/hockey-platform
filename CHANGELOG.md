...
- `history.json`: добавлен роадмап из 9 глав (1 опубликована, 8 — заглушки `published:false`) с полями `n/era/kicker` (тизеры из утверждённого макета; текст глав — research-чат)
...
- Старый `docs/app.js` (программка-словарь) осушён переездом на `dictionary.html`; `style.css` оставлен (его читает `origins.html`)
