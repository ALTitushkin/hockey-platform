...
### Добавлено (глава 5 опубликована)
- `docs/history/summit-1972.html` — глава 5 «Суперсерия-1972: восемь матчей, которые изменили хоккей», собрана `tools/build_chapter.py`: breadcrumb (Главная › История › Суперсерия-1972), chapter-label «Глава 5 · История хоккея», 8 разделов с id-якорями (`prologue`, `rosters`, `game-1`, `games-2-4`, `turning-point`, `game-6-slash`, `games-7-8`, `aftermath`), 5 fact-box, таймлайн (8 дат), «Источники» (6 ссылок), nav-footer (← Тарасов и большая система; `canada-cup` задизейблена), видимый `dateModified`
- JSON-LD `Article` (даты `2026-07-05T12:00:00+03:00`) + `BreadcrumbList`
- `data/history.json` и `docs/data/history.json`: `summit-1972` → `published:true`, `url`, `summary` (из «Краткого резюме»), `sections[]` (8); `era`(soviet)/порядок/`kicker` («Встреча миров») не тронуты
- `sitemap.xml` — 8 URL (5 опубликованных глав)

### Изменено (тулинг)
- `tools/build_chapter.py`: парсер «Источники» рендерит несколько ссылок в одной строке (`[текст](url) и [текст](url) — описание`) — один источник остаётся одним `<li>`; обратно совместимо с одиночной ссылкой + описанием
...
- `project/planning/sprint_3.md` — ТЗ спринта 3
...
- `project/planning/sprint_2.md` — ТЗ спринта 2
- `project/planning/results/sprint_1_results.md` — итоги спринта 1
