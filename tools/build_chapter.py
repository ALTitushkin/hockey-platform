...
CONFIG = {
    "id": "tarasov-system",
    "date": "2026-06-28",                       # дата публикации/обновления (date-only)
    "h1": "Тарасов и большая система",
    "subtitle": "Наука, поточный метод и пятёрка как один организм",
    "chapter_label": "Глава 4 · История хоккея",
    "breadcrumb_name": "Тарасов и большая система",
    "og_title": "Тарасов и большая система — история советского хоккея",
    "og_desc": "Как Анатолий Тарасов поставил хоккей на научную основу: поточный метод, пятёрка как один организм, круглогодичная подготовка и тандем с Чернышёвым.",
    "meta_desc": "Метод Анатолия Тарасова: интенсификация и поточный метод, пятёрка как единый организм, наука в подготовке, кросс-тренинг и девятилетняя победная серия сборной СССР. Глава 4 истории хоккея.",
    "article_desc": "Метод Анатолия Тарасова: поточный метод, пятёрка как один организм, круглогодичная научная подготовка, тандем с Чернышёвым и девятилетняя серия побед СССР (1963–1971).",
    "page_title": "Тарасов и большая система · Культура хоккея",
    "section_slugs": ["engineer","five-as-one","year-round",
                      "cross-training","legends-vs-facts","tandem"],
    "prev": ("soviet-origins.html", "← С чистого льда"),
    "next_label": "Суперсерия-1972 →",          # следующая глава — пока задизейблена
    "typo_fixes": [],
}
# ──────────────────────────────────────────────────────────────────────────────────
    ...
    while i<n:
        ...
        if b.startswith("## Источники"):
            ...
            for ln in blocks[i+1].split("\n"):
                ...
                if m:
                    li=f'<a href="{m.group(2)}" rel="noopener" target="_blank">{esc(m.group(1))}</a>'
                    if m.group(3):
                        li+=f' — {inline(m.group(3))}'
                    lis.append(f'\n        <li>{li}</li>')
